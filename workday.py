...
EXCLUDE_TITLE_RE = re.compile(r"\b(senior|advanced|staff|manager|lead|principal|sr|software engineer ii|software engineer iii|embedded|software engineer 2|software engineer 3|software engineer iv)\b", re.IGNORECASE)
...
# ----------------------------
# Extract requisition token for /details/ reconstruction
# ----------------------------
...
# ----------------------------
# Rewrite Workday /job/ links to force en-US and remove location segment
# ----------------------------
def _rewrite_job_url_to_en_us_site(tenant: WorkdayTenant, raw_url: str) -> str:
    """
    Converts:
      https://host/job/GA-Atlanta/Some-Job_JR2506076
    Into:
      https://host/en-US/<site>/job/Some-Job_JR2506076

    Also handles:
      /job/GA-Atlanta/Some-Job_JR...
      /job/Some-Job_JR...
    Preserves the querystring if present.
    """
    try:
        u = urlparse(raw_url)
        ...
        if not parts or parts[0] != "job":
            return raw_url

        # Workday sometimes has /job/<location>/<slug> or /job/<slug>
        job_slug = parts[-1] if len(parts) >= 2 else ""
        if not job_slug:
            return raw_url

        site = tenant.site_slug or "ExternalCareerSite"
        query = f"?{u.query}" if u.query else ""
        return f"{host_base}/en-US/{site}/job/{job_slug}{query}"
    except Exception:
        return raw_url

# ----------------------------
# Correct Workday job URL builder
# ----------------------------
def workday_details_url(tenant: WorkdayTenant, job: Dict[str, Any]) -> str:
    """
    URL preference:
      1) Use API-provided /details/ URL if present
      2) Else use API-provided /job/ URL if present, but rewrite to /en-US/<site>/job/<slug>
    ...
    for key in ("externalPath", "jobPath", "path"):
        ...
        if p.startswith("http"):
            if "/details/" in p:
                return p
            if "/job/" in p:
                return _rewrite_job_url_to_en_us_site(tenant, p)

        # Relative URL
        if p.startswith("/"):
            full = tenant.api_base + p
            if "/details/" in p:
                return full
            if "/job/" in p:
                return _rewrite_job_url_to_en_us_site(tenant, full)

        # Odd relative (no leading slash)
        if "/details/" in p or "/job/" in p:
            full = tenant.api_base + ("/" + p.lstrip("/"))
            if "/details/" in p:
                return full
            return _rewrite_job_url_to_en_us_site(tenant, full)

    # Reconstruct /details/ if no /details/ or /job/ was provided
